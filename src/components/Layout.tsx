...
import { Bell, Settings, LogOut, FileText } from 'lucide-react'
    ...
    <div className="flex min-h-screen bg-gray-50">
      ...
      <div className="flex-1 flex flex-col min-w-0">
        ...
        <header className="bg-white border-b border-gray-100 px-8 py-4 flex items-center gap-4 sticky top-0 z-10">
          <h1 className="text-base font-semibold text-[#0a2741] shrink-0">{title}</h1>
          ...
          <div className="flex items-center gap-4 ml-auto">
            <div className="relative" ref={bellRef}>
              ...
              >
                <Bell size={20} className="text-gray-400 hover:text-[#0a2741] transition" />
                {unreadCount > 0 && (
              ...
              {bellOpen && (
                <div className="absolute right-0 top-10 w-80 bg-white rounded-2xl shadow-xl border border-gray-100 z-50 overflow-hidden">
                  <div className="px-4 py-3 border-b border-gray-100 flex items-center justify-between">
                    <p className="text-sm font-bold text-[#0a2741]">Notifications</p>
                    {notifications.length > 0 && (
                      <button onClick={markAllRead} className="text-xs text-gray-400 hover:text-[#0a2741] cursor-pointer">
                        Tout marquer lu
                      </button>
                    )}
                  </div>
                    ...
                    {notifications.length === 0 ? (
                      ...
                      notifications.map((n: AdminNotification) => (
                        <div key={n.id} className={`flex items-start gap-3 px-4 py-3 border-b border-gray-50 last:border-0 ${n.read ? '' : 'bg-blue-50/50'}`}>
                          <div className="w-8 h-8 rounded-full bg-[#0a2741] flex items-center justify-center shrink-0 mt-0.5">
                            <FileText size={14} className="text-[#cdf900]" />
                          </div>
                          <div className="flex-1 min-w-0">
                            <p className="text-sm font-semibold text-[#0a2741]">{n.message}</p>
            ...
            <div className="relative" ref={menuRef}>
              ...
              >
                <div className="text-right">
                  <p className="text-sm font-semibold text-[#0a2741] leading-tight">{user ? `${user.firstName} ${user.lastName}` : 'Admin'}</p>
                  <p className="text-xs text-gray-400">{user?.role ?? 'Super Admin'}</p>
                </div>
                <div className="w-9 h-9 rounded-full bg-[#0a2741] flex items-center justify-center shrink-0 hover:opacity-80 transition">
                  <span className="text-[#cdf900] text-sm font-bold">{user?.firstName?.[0] ?? 'A'}</span>
                </div>
              </button>
              ...
              {menuOpen && (
                <div className="absolute right-0 top-12 w-52 bg-white rounded-2xl shadow-xl border border-gray-100 py-2 z-50">
                  <div className="px-4 py-2 border-b border-gray-50 mb-1">
                    <p className="text-xs font-semibold text-[#0a2741] truncate">{user?.firstName} {user?.lastName}</p>
                    <p className="text-xs text-gray-400 truncate">{user?.email}</p>
                  </div>
                  <button
                    onClick={() => { setMenuOpen(false); setProfileOpen(true) }}
                    className="w-full flex items-center gap-3 px-4 py-2.5 text-sm text-[#0a2741] hover:bg-gray-50 transition cursor-pointer"
                  >
